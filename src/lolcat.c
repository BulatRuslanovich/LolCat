...
const char* escStNames[EST_COUNT] = {
	[NONE] = "NONE",
	[ESC_BEGIN] = "BEGIN",
	[ESC_STRING] = "STRING",
	[ESC_CSI] = "CSI",
...
int initStuct(Flags *flags, int symbol, int *seed, double *freq_h, double *freq_v, int *startColor, union rgb_c *rgb_start, union rgb_c *rgb_end) {
	int errCode = OK;
	char *endPtr;
...
int main(int argc, char** argv) {
	...
	int seed = time(NULL);
	int errCode = lol;
	Flags flags = {false, true, false, false, false, false, false};
	...
	while (((flagSymbol = getopt_long(argc, argv, flagsString, longFlags, NULL)) != -1)) {
		errCode = initStuct(&flags, flagSymbol, &seed, &freq_h, &freq_v, &startColor, &rgb_start, &rgb_end);
	}
			...
			if (colors) {
				...
				if (escapeState == NONE || escapeState == ESC_CSI_TERM) {
					if (c == '\n') {
						...
						if (flags.b) {
							float theta = i * freq_h / 5.0f + l * freq_v + PI * (offX + 2.0f * (randomOffset + startColor) / RAND_MAX);
							...
							if (flags.g) {
								...
								float offset = 0.1;
								color.r = lrintf((offset + (1.0f - offset) * (0.5f + 0.5f * sin(theta))) * 255.0f);
								color.g = lrintf((offset + (1.0f - offset) * (0.5f + 0.5f * sin(theta + 2 * PI / 3))) * 255.0f);
								color.b = lrintf((offset + (1.0f - offset) * (0.5f + 0.5f * sin(theta + 4 * PI / 3))) * 255.0f);
							}
							...
							if (cc != ncc || escapeState == ESC_CSI_TERM) {
								wprintf(L"\033[%hhum", (flags.i ? 10 : 0) + codes16[(randomOffset + startColor + (cc = ncc)) % ARRAY_SIZE(codes16)]);
							}
							...
							if (flags.g) {
								...
								if (cc != ncc || escapeState == ESC_CSI_TERM) {
									size_t lookup = (randomOffset + startColor + (cc = ncc)) % (2 * ARRAY_SIZE(codesGradient));
								...
								if (cc != ncc || escapeState == ESC_CSI_TERM) {
									wprintf(L"\033[%d;5;%hhum", (flags.i ? 48 : 38), codes[(randomOffset + startColor + (cc = ncc)) % ARRAY_SIZE(codes)]);
								}
							}
						}
					}
